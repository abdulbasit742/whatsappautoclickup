...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    for (const msg of msgs) {
      ...
      await upsertConversation(client.id);

      await db.query(`UPDATE clients SET last_active_at=NOW(), last_replied_at=NOW() WHERE id=$1`, [client.id]);
      await refreshLeadScore(client.id).catch(() => {});
      ...
      if (hour < sh || hour >= eh) {
        ...
        await sendAndLogMessage(client.id, from, offlineMsg.rows[0]?.value || 'We are currently offline. We will reply soon!');
        continue;
      }
      ...
      if (lc.includes('book') || lc.includes('call') || lc.includes('meeting')) {
        await sendAndLogMessage(client.id, from, `To book a call, please tell us your preferred date and time and we will confirm a slot for you! 📅`);
        continue;
      }
...
async function handleOnboarding(client, to, isNew) {
  ...
  await sendAndLogMessage(client.id, to, greeting);
  if (isNew) {
    await db.query(
...
async function handlePricing(client, to) {
  ...
  await sendAndLogMessage(client.id, to, msg);
}
...
async function handleReview(client, to, rating) {
  ...
  await sendAndLogMessage(client.id, to, `${stars} Thank you for your rating! Your feedback means a lot to us. 🙏`);
  if (rating >= 4) {
    await sendAndLogMessage(client.id, to, `We're so glad you had a great experience! Would you like to try any of our other services? Type *pricing* to see options. 🚀`);
  }
}
...
Do not make up prices or services not listed above.`;
  ...
  if (!success || !response) {
    await sendAndLogMessage(client.id, to, `I'm having a moment — let me connect you with our team right away! 🙏`);
    await db.query(
  ...
  await sendAndLogMessage(client.id, to, response, providerUsed, shouldFlag);
}

async function sendAndLogMessage(clientId, to, content, providerUsed = null, isFlagged = false) {
  ...
  await db.query(
    ...
    [clientId, content, providerUsed, isFlagged]
  );
  await db.query(
...
async function upsertConversation(clientId) {
  await db.query(
    `INSERT INTO conversations (client_id, status, last_message_at, unread_count)
     VALUES ($1,'open',NOW(),1)
