...
const { enqueueMessageJob } = require('./jobQueueService');
    ...
    for (const b of broadcasts) {
      ...
      for (const c of clients) {
        ...
        await enqueueMessageJob({
          ...
          clientId: c.id,
          broadcastId: b.id,
          payload: { to: c.whatsapp_number, message: msg }
        });
        await db.query(
      ...
      await db.query(`UPDATE broadcasts SET status='running', total_sent=$1 WHERE id=$2`, [queued, b.id]);
      console.log(`[Cron] Broadcast "${b.title}" queued for ${queued} clients`);
    }
  } catch (err) {
