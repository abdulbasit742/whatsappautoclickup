...
CREATE TABLE clients (
  ...
  lead_temperature VARCHAR(10) DEFAULT 'cold' CHECK (lead_temperature IN ('hot','warm','cold')),
  tags             TEXT[] DEFAULT ARRAY[]::TEXT[],
  last_replied_at  TIMESTAMPTZ,
  is_invalid       BOOLEAN DEFAULT FALSE,
  merged_to_id     UUID REFERENCES clients(id),
...
CREATE TABLE users (
  ...
  role          VARCHAR(30) NOT NULL DEFAULT 'agent' CHECK (role IN ('owner','admin','manager','agent')),
...
CREATE INDEX idx_contact_tags_client ON contact_tags(client_id);
...
CREATE TABLE broadcasts (
  ...
  status          VARCHAR(20) DEFAULT 'draft' CHECK (status IN ('draft','scheduled','running','paused','sent','failed','archived')),
...
-- ─── FOLLOW-UP AUTOMATION STEPS (DRIP) ─────────────────────────────────────────
CREATE TABLE follow_up_sequences (
  id             UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name           VARCHAR(100) NOT NULL,
  trigger_type   VARCHAR(30) NOT NULL CHECK (trigger_type IN ('no_reply','seen_no_reply','manual')),
  is_active      BOOLEAN DEFAULT TRUE,
  created_at     TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE INDEX idx_followup_steps_sequence ON follow_up_sequence_steps(sequence_id, sort_order);

-- ─── QUEUED MESSAGE JOBS ───────────────────────────────────────────────────────
CREATE TABLE message_jobs (
  id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  type            VARCHAR(30) NOT NULL CHECK (type IN ('broadcast','direct','automation')),
...
CREATE INDEX idx_message_jobs_queue ON message_jobs(status, run_after);
